...
{
    public class BTaggerDropDownMenuWindow : EditorWindow
    {
        ...
        void OnEnable()
        {
            ...
            menu = AssetDatabase.LoadAssetAtPath<VisualTreeAsset>(localPath + "/BTaggedMenuUI.uxml");
            menuCategory = AssetDatabase.LoadAssetAtPath<VisualTreeAsset>(localPath + "/BTaggedMenuCategoryUI.uxml");
            menuItem = AssetDatabase.LoadAssetAtPath<VisualTreeAsset>(localPath + "/BTaggedMenuItemUI.uxml");
        ...
        public void Build(List<(string label, int idx, bool editable)> _menuEntries, float windowWidth)
        {
            ...
            categoriesPerEntry = new List<TemplateContainer>[menuEntries.Count];

            // First entry is always empty - 'None'
            var firstEntry = CreateNestedItem(menuEntries, 0, 0);
            menuScrollView.Add(firstEntry);
            VisualElement contentGroup = new VisualElement();
            
            contentGroup.style.maxWidth = windowWidth - 15f;
            contentGroup.AddToClassList("btagged-group");
            ...
            {
                contentGroup.Add(CreateNestedItem(menuEntries, i, 0));
            }
            ...
            newGroupBtn.text = menuEntries[menuEntries.Count - 1].label;
            newGroupBtn.clicked += () => AddCategory(menuEntries[menuEntries.Count - 1].idx, NewGroupLabel);
            menuScrollView.Add(newGroupBtn);

            rootVisualElement.style.borderBottomColor = rootVisualElement.style.borderTopColor = rootVisualElement.style.borderLeftColor = rootVisualElement.style.borderRightColor = Color.black;
            rootVisualElement.style.borderBottomWidth = rootVisualElement.style.borderTopWidth = rootVisualElement.style.borderLeftWidth = rootVisualElement.style.borderRightWidth = 1f;
        ...
        private VisualElement CreateNestedItem(List<(string label, int index, bool editable)> menuEntries, int idx, int depth)
        {
            ...
            if (depth < splitLabels.Length-1)
            {
                ...
                else
                {
...
#if UNITY_2020_1_OR_NEWER
                    category.Q<Button>("create").RegisterCallback<ClickEvent>(ce => { ce.StopPropagation(); AddCategory(menuEntryIndex, thisLabel); });
                    category.Q<Button>("edit").RegisterCallback<ClickEvent>(ce => { ce.StopPropagation(); EditCategory(menuEntryIndex, depth, thisLabel); });
                    category.Q<Button>("delete").RegisterCallback<ClickEvent>(ce => { ce.StopPropagation(); DeleteCategory(menuEntryIndex, depth, thisLabel); });
#else
                        ...
                        if (ce.newValue)
                        {
                            ...
                            menuScrollView.schedule.Execute(() =>
                            {
                                if(menuScrollView.verticalScroller.enabledSelf)
                                {
                                    ...
                                    //Debug.LogWarning("Scrolling to " + pos + " for " + itemFoldout.worldBound);
                                }
                            }).ExecuteLater(50);
                        }
                        else
                    ...
                    if (createdCategories.ContainsKey((depth, key))) { Debug.LogAssertion("Already contains " + key + " for level " + depth); };
...
#if UNITY_2020_1_OR_NEWER
                item.Q<Button>("edit").RegisterCallback<ClickEvent>(ce => { ce.StopPropagation(); Edit(menuEntryIndex); });
                item.Q<Button>("duplicate").RegisterCallback<ClickEvent>(ce => { ce.StopPropagation(); Duplicate(menuEntryIndex); });
                item.Q<Button>("delete").RegisterCallback<ClickEvent>(ce => { ce.StopPropagation(); Delete(menuEntryIndex); });
#else
        ...
        {
            menuScrollView.Query<Foldout>().ForEach(x =>
            {
            ...
            for (int c = 0; c < categoriesPerEntry.Length; ++c)
            {
                ...
                for(int catIdx = 0; catIdx < cats.Count; ++catIdx)
                {
                    ...
                    {
                        bool isSelectedEntry = (WorkingSelectedEntry >= 0 && WorkingSelectedEntry == menuEntryIndex) || (WorkingSelectedEntry < 0 && SelectedEntry == menuEntryIndex);
                        if(isSelectedEntry)
                        {
        ...
        private void AddCategory(int idx, string label)
        {
            OnAddCategory?.Invoke(RelatedProperty, idx, label);
        }
        private void EditCategory(int idx, int depth, string label)
        {
            OnEditCategory?.Invoke(RelatedProperty, idx, depth, label);
        }
        private void DeleteCategory(int idx, int depth, string label)
        {
            OnDeleteCategory?.Invoke(RelatedProperty, idx, depth, label);
        }
        private void Select(int idx)
        {
            OnSelect?.Invoke(RelatedProperty, idx);
        }
        private void Duplicate(int idx)
        {
            OnDuplicate?.Invoke(RelatedProperty, idx);
        }
        private void Delete(int idx)
        {
            OnDelete?.Invoke(RelatedProperty, idx);
        }
        private void Edit(int idx)
        {
            OnEdit?.Invoke(RelatedProperty, idx);
        }
    }
}
