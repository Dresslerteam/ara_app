...
{
    static public class BTaggedSettingsProvider
    {
        ...
        public static SettingsProvider CreateMyCustomSettingsProvider()
        {
            var provider = new SettingsProvider("Preferences/BTaggedSettings", SettingsScope.User)
            {
                ...
                activateHandler = (searchContext, rootElement) =>
                {
                    ...
                    showGroupLabelsToggle.RegisterValueChangedCallback(ce => { settings.FindProperty("showGroupNames").boolValue = ce.newValue; settings.ApplyModifiedProperties(); });
                    ...
                    showHashesToggle.RegisterValueChangedCallback(ce => { settings.FindProperty("showHashes").boolValue = ce.newValue; settings.ApplyModifiedProperties(); });
                    contents.Add(showHashesToggle);

                    Button btn = new Button(() => CheckForCollisions(true));
                    btn.tooltip = "In rare circumstances - such as a bad merge from source control - BTaggedSO's may not have unique identifiers. If ever in doubt, click this to check all assets and note the output in the console.";
                    btn.text = "Check for Duplicated Assets";
                    contents.Add(btn);

#if ODIN_INSPECTOR
                    Toggle odinSOToggle = new Toggle("Use Odin SerializedScriptableObject");
                    odinSOToggle.tooltip = "By default BTaggedSO inherits from ScriptableObject. Enable this option if you have Odin installed to inherit from Odin's SerializedScriptableObject instead.";
                    odinSOToggle.RegisterValueChangedCallback(cb => 
                    {
                        if(cb.newValue != cb.previousValue)
                        {
                            if (cb.newValue) AddCompileDefine("BTAGGED_ODIN_BASE", new BuildTargetGroup[] { EditorUserBuildSettings.selectedBuildTargetGroup });
                            if (!cb.newValue) RemoveCompileDefine("BTAGGED_ODIN_BASE", new BuildTargetGroup[] { EditorUserBuildSettings.selectedBuildTargetGroup });
                            settings.FindProperty("useOdinSO").boolValue = cb.newValue;
                            settings.ApplyModifiedProperties();
                        }
                    });
                    odinSOToggle.value = settings.FindProperty("useOdinSO").boolValue;
                    contents.Add(odinSOToggle);
#endif
        ...
        {
            BTaggedSORegistry.FindAllAssets<BTaggedGroupBase, BTaggedSO>();
            ...
            for (int i = 0; i < BTaggedSORegistry.AllAssets.Count; ++i)
            {
                ...
                {
                    //Debug.Log("Asset " + i + " hash: " + asset.Hash);
                    if (set.Contains(asset.Hash))
                    {
                        var dupHash = asset.Hash;
                        while(set.Contains(asset.Hash)) asset.EditorGenerateNewHash();
                        if (BTaggedSORegistry.AllAssets[i].group != null) AssetDatabase.SetMainObject(BTaggedSORegistry.AllAssets[i].group, AssetDatabase.GetAssetPath(asset));
                        EditorUtility.SetDirty(asset);
                        hasDuplicates = true;
                        set.Add(asset.Hash);
                        //Debug.LogWarning(asset + " was a duplicate with hash " + dupHash + " so generated new hash:"+ asset.Hash+".\n");
                    }
                    else
                    {
                        set.Add(asset.Hash);
            ...
            willCheck = false;
            if (userInitiated && !hasDuplicates) Debug.Log("All assets inheriting from BTaggedSO appear to have unique hashes. Checked " + set.Count + " assets and No Duplicates were detected.");
        }
